...
export const buildNewLine = (quantity: number): string => NEW_LINE.repeat(quantity);

export const buildLine = (content: string): string => buildEmptyMessage()
    .concat(TEXT_RESET)
    .concat(content)
    .concat(TEXT_RESET);

export const buildLogo = (bgColor: string, logo: string): string => buildEmptyMessage()
    .concat(TEXT_BRIGHT)
    .concat(bgColor)
    .concat(SPACE)
    .concat(logo)
