...
import {GetSystemProgramUrlService} from "../dom-service/get-system-program-url.service";
...
export class SystemProgramExistAppService {
    ...
    run(programName: SystemProgramEnum): boolean {
        this.simpleMessage.writePlain(`Step: System Program Exist >>> ${programName}`);
        if (this.systemProgramExist.checkExist(programName)) {
            return true;
        }
        this.newline.writeNewline();
        const url = this.getSystemProgramUrl.getUrl(programName);
        this.simpleMessage.writeError(`The ${programName} program does not exist on the system`);
