...
export class ValidatorCorrectOrderService implements ParamDtoValidatorModel {
    constructor(
    ...
    run(): ParamDtoValidationModel {
        ...
        const wrongParamsDto = paramDto.params.filter(param => !this.checkParamOrder(param, program));
    ...
    private checkParamOrder(paramDto: ParamDtoEntityModel, program: ParamDtoEntityModel | undefined): boolean {
        const paramOrder = this.getParamOrder().find(order => order.paramTypes.includes(paramDto.type));
        if (paramOrder == null) throw new Error("Not supported param type!");
        if (paramOrder.order === 3 && (program == null)) return paramDto.index >= 2;
        if (paramOrder.order === 3 && (program != null)) return paramDto.index >= paramOrder.order;
    ...
    private getParamOrder(): Array<{ order: number; paramTypes: ParamTypeEnum[]; }> {
        return [
            ...
            {order: 3, paramTypes: [ParamTypeEnum.command, ParamTypeEnum.argument, ParamTypeEnum.alias]}
        ];
    }
}
