...
export class GenerateWsStructureService {
        ...
        wsStructureModels.forEach(wsStructureModel => {
            switch (wsStructureModel.type) {
                ...
                case WsStructureEnum.createTsconfigJsonFile:
                    ...
                    break;
                default:
                    throw new Error(`Not supported workspace structure entity!`);
            }
        });
    }
    ...
    private createTsconfigJsonFile(
        ...
        if (this.pathUtils.existPath(tsconfigJsonPath)) {
            ...
            this.fileUtils.writeJsonFile(tsconfigJsonPath, tsconfigJsonNewContent);
        } else {
            const packageJsonNewContent: WsTsconfigDtoModel = this.buildTsconfigJson.build();
            this.fileUtils.writeJsonFile(tsconfigJsonPath, packageJsonNewContent);
        }
        this.processGenerateStructure(wsStructureModel.children);
    }
}
