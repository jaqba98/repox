...
import {ConvertProjectTypeService} from "../converter/convert-project-type.service";
...
export class BuildProjectPathService {
    ...
    buildPath(name: string, type: string, path: string): string {
        if (path === EMPTY_STRING) {
            ...
            const workspaceType = this.convertProjectType.toWorkspaceFolder(projectType);
            return this.pathUtils.createPath(WorkspaceFolderEnum.projects, workspaceType, name);
        }
